...
//    std::vector<float> landmark_positions {5, 10, 20};
    std::vector<float> landmark_positions {10};
...
float motion_model(float pseudo_position, float movement, std::vector<float> priors,
    ...
    //YOUR CODE HERE
    for (int pre_pseudo_position = 0; pre_pseudo_position < map_size; pre_pseudo_position++) {
        float delta = (pseudo_position - pre_pseudo_position);
       position_prob += priors[pre_pseudo_position] * Helpers::normpdf(delta, movement, control_stdev);
    }
